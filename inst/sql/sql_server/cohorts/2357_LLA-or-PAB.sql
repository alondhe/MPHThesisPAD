...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (4219032)and invalid_reason is null
...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (4295278)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (4295278)
  and c.invalid_reason is null
...
where po.procedure_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 25)
) C
...
(
  select po.*, ROW_NUMBER() over (PARTITION BY po.person_id ORDER BY po.procedure_date, po.procedure_occurrence_id) as ordinal
  FROM @cdm_database_schema.PROCEDURE_OCCURRENCE po
where po.procedure_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 28)
) C
