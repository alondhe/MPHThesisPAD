...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (40756872,40757057,40756966)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (40756872,40757057,40756966)
  and c.invalid_reason is null
...
WHERE C.ordinal = 1
AND (C.procedure_date >= DATEFROMPARTS(2011, 01, 01) and C.procedure_date <= DATEFROMPARTS(2016, 12, 31))
